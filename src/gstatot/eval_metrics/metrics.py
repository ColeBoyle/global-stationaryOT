...
class Metric_Evaluator:
    ...
    def __init__(self, method, test_adata, true_adata, time_key, 
        ...
        self.results_df = pd.DataFrame(columns=['method', 'Age', 'marginal_W2_dist', 
                                                'traj_W2_dist', 'FP_TV_dist', 'FP_test_nans', 'prop_TV_dist'])


        self.times = jnp.unique(jnp.asarray(self.test_adata.obs[self.time_key], dtype=np.float32))
    ...
    def w2_marginal_error(self, get_dist=True, gwot=False):
        ...
        for i in range(self.T):
            ...
            if self.full_supp:
                ...
                test_marginal = test_marginal / test_marginal.sum()
    ...
    def w2_trajectory_error(self, test_dt, true_dt, num_traj=None):
        true_dist = [self.true_adata.uns['true_traj_data'][str(t)] for t in self.times]
        if test_dt >= true_dt:
        ...
        for i in range(self.T):
            ...
            traj_data = self.test_adata.uns[f'{self.method}_traj_data'][str(self.times[i])]
    ...
    def fp_tv_error(self, label_key, plot_lin_fp=False, true_key=None, plot_corr=False):
        if true_key is None:
        ...
        for i in range(self.T):
            ...
            # record if nans in model_FP_i
            FP_nans.append(jnp.isnan(model_FP_i).any())
            ...
            TV_distance =jnp.abs(true_FP_i - model_FP_i).sum(axis=1) * 0.5
...
#            model_FP_i = jnp.where(jnp.isnan(model_FP_i), -1, model_FP_i)
            ...
            if np.any(jnp.isnan(model_FP_i)) or np.any(jnp.isnan(true_FP_i)):
                perc_matches.append(np.nan)
                continue
        ...
        self.results_df['FP_test_nans'] = FP_nans

        if plot_corr:
            n_cols = true_FPS[0].shape[1]
            corrs = np.zeros((self.T, n_cols))
            ...
            for i in range(self.T):
                for j in range(n_cols):
                    ...
                    corrs[i, j] = corr
                    ax.text(0.1, 0.9, f'Corr: {corr:.2f}', 
                    ...
                    ax.grid()
            self.test_adata.uns['FP_lin_corr'] = corrs
            plt.tight_layout()
            if self.exp_dir is not None:
    ...
    def prop_TV_error(self, type_key):
        if self.full_supp:
            cell_types = self.test_adata.obs[type_key].values
            gr = self.test_adata.obs['growth_rate'].values
            cell_types = cell_types[gr<0]
            unique_types = np.unique(cell_types)
            prop_dict = {type_: [] for type_ in unique_types}
            for age in self.times:
                pi = self.test_adata.obsp[f'pi_{age}']
                test_marginal = np.asarray(pi.sum(axis=0), dtype=np.float32)
                test_marginal = test_marginal[gr<0]
                test_marginal = test_marginal / test_marginal.sum()
                for type_ in unique_types:
                    prop_dict[type_].append(test_marginal[cell_types == type_].sum())
            test_props = pd.DataFrame(prop_dict, index=self.times)
        else:
            test_props = pd.crosstab(self.test_adata.obs[self.time_key], self.test_adata.obs[type_key], normalize='index')

        true_adata_subset = self.true_adata[self.true_adata.obs[self.time_key].isin(np.unique(self.test_adata.obs[self.time_key]))]
        true_adata_subset = true_adata_subset[true_adata_subset.obs['growth_rate'] < 0]
        true_props = pd.crosstab(true_adata_subset.obs[self.time_key], true_adata_subset.obs[type_key], normalize='index')
        TV_dist = np.abs(test_props - true_props).sum(axis=1)
        self.results_df['prop_TV_dist'] = TV_dist.values

        if self.plot_metrics:
            fig, ax = plt.subplots(figsize=(5, 5))
            sns.lineplot(data=self.results_df, x='Age', y='prop_TV_dist', ax=ax, marker='o')
            ax.grid()
            plt.xlabel('Age')
            plt.ylabel('TV distance')
            ax.set_title(f'{self.method}\nTV distance between true and test proportions')

            if self.exp_dir is not None:
                fig.savefig(os.path.join(self.exp_dir, f'{self.method}_prop_TV_distance.png'), bbox_inches="tight")
            else:
                plt.show()
